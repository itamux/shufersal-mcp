...
test('catalog browser contracts, pagination and account-field minimization', async () => {
  ...
  try {
    ...
    page.on('request',async r=>{
      ...
      if(u.pathname.endsWith('/results')) {contentType='application/json';body=JSON.stringify({results:[{code:'P_1',name:'Milk',baseProductImageMedium:'https://res.cloudinary.com/shufersal/image/upload/v1/product.png',secret:'omit'}],pagination:{currentPage:ignored?0:Number(u.searchParams.get('page')),pageSize:5,sort:'relevance',totalNumberOfResults:12,numberOfPages:3}});}
