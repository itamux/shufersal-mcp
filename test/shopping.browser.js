...
for (const outcome of ['success', 'rejected-write', 'stale-cart', 'editing-order', 'expired-session']) {
    ...
    const launch=async options=>{
      ...
      browser.newPage=async()=>{const page=await original();page.on('request',request=>{
        request.continue=async({headers})=>{
          ...
          if(url.pathname.includes('/cart/')) body=`<div id="cartTotalItems">${qty}</div><div class="discountCartBottom">₪10</div>`+(qty?`<div class="miglog-prod" data-entry-number="0" data-product-code="P_123" data-entry-qty="${qty}" data-product-name="Fixture"><input name="sellingMethod" value="BY_UNIT"></div>`:'');
