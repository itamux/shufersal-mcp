...
for(const scenario of ['recover','rejected','already-authenticated','write-failure','restore-existing']) {
  test(`automatic session: ${scenario}`,async()=>{
    let authenticated=['already-authenticated','restore-existing'].includes(scenario),logins=0,writes=0,qty=0;
    const launch=async options=>{
      ...
      browser.newPage=async()=>{
        ...
        page.on('request',r=>{r.continue=async()=>{
          ...
          if(scenario==='restore-existing'&&u.search==='?restoreCart=true')qty=4;
          const cart=`<span id="cartTotalItems">${qty}</span>`+(qty?`<div class="miglog-prod" data-entry-number="0" data-product-code="P_1" data-entry-qty="${qty}" data-selling-method="BY_UNIT"></div>`:'');
    ...
    try{
      ...
      if(scenario==='restore-existing'){
        assert.equal((await s.cart()).items[0].quantity,4);
        assert.equal(logins,0);assert.equal(writes,0);return;
      }
