...
tool('search_shufersal', 'Search products with server-side paging, category, brand, available facets and sorting. Facets describe available filter codes. Does not change the cart.', { ...catalog, query: z.string().trim().min(1).max(200).regex(/^[^:\x00-\x1f]+$/) }, args => shopping.search(args));
...
tool('browse_shufersal_category', 'Browse products in a category or subcategory returned by get_shufersal_categories, with paging, sorting and filters.', { ...catalog, category_code: code }, args => shopping.search(args));
